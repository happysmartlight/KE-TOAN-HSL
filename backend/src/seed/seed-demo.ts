...
export type SeedOptions = {
  /** Nếu có, không xóa user này (để admin đang đăng nhập không bị logout) */
  preserveAdminId?: number;
  /** In log ra console (mặc định true cho CLI, false cho runtime) */
  verbose?: boolean;
};

// ============================================================
// MAIN — Có thể gọi từ CLI (npm run db:seed) HOẶC từ admin runtime
// ============================================================
export async function seedDemoData(prisma: PrismaClient, opts: SeedOptions = {}) {
  const { preserveAdminId, verbose = true } = opts;
  ...
  // Lấy username của admin đang đăng nhập (nếu có) để tránh trùng với staff seed
  ...
  // ============================================================
  log('🧾  Tạo hóa đơn bán hàng (KPI distribution)...');
  ...
  // ============================================================
  log('💸  Tạo bút toán thu/chi định kỳ...');
  ...
  log('     thuylinh / staff123   (Trần Thùy Linh - top sales)');
  log('     minhquan / staff123   (Lê Minh Quân)');
  log('     hoangnam / staff123   (Phạm Hoàng Nam)');
  log('     thuhuong / staff123   (Đỗ Thu Hương)');
  log('     kimanh   / staff123   (Vũ Kim Anh - mới vào)');
  log('     oldnv    / staff123   (Hoàng Văn Cũ - đã nghỉ)\n');

  return counts;
}

// CLI entrypoint nằm ở backend/prisma/seed.ts
